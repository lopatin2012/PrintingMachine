...
| `services/` | Async print queue (`print_queue.py`), ZPL rendering (`zpl_renderer.py`), local ZPL→PNG renderer (`zpl_pil_renderer.py`), preview chain (`preview_renderer.py`), external DataMatrix client (`datamatrix_service.py`), own DataMatrix ECC200 engine (`datamatrix_renderer.py`), local PDF builder (`pdf_renderer.py`) |
...
- `services/zpl_pil_renderer.py` — Local ZPL→PNG renderer (PIL + python-barcode + own DataMatrix engine); the offline bottom line, not the default
- `services/datamatrix_renderer.py` — Own DataMatrix ECC200 engine (pure Python: ASCII compaction, Reed-Solomon GF(256) 0x12D, ISO/IEC 16022 placement, finder/clock assembly); replaces pylibdmtx for rendering
- `services/preview_renderer.py` — Preview chain (Labelary → zplr(Node) → PIL); used by `/templates/preview*`. `render_preview_png_local()` is the Labelary-free variant used by the PDF builder
- `services/pdf_renderer.py` — Builds a multi-page PDF (one page per box, with per-box DataMatrix codes) fully locally; used by `POST /printing/start` with `printer_id=__pdf__`
...
- **Preview rendering:** `/templates/preview/render` (what the UI actually calls) and `/templates/preview` render PNG through a chain in `services/preview_renderer.py` (`render_preview_png()`, header `X-Render-Engine`): **1) Labelary** (external HTTP, the look you want), **2) local `zplr`** via a Node bridge `zplr_preview_server.mjs` (geometry/barcodes like Labelary, Cyrillic via `RobotoCondensed-Bold.ttf` — **optional**: needs `node` + `npm i` for `zplr`, `skia-canvas`; skips silently if absent), **3) PIL** `render_zpl_to_png()` as guaranteed fallback. `/templates/preview/render_local` is the **legacy** route — it goes through `render_zpl_preview()` (`services/zpl_renderer.py`), which tries zebrash → zebrafy → PIL fallback; its output is also the PIL renderer in practice. **`bin/zebrash-renderer` is NOT runnable** (a Go package archive `.a`, not an executable; running it fails) — the zebrash path is dead code, don't try to fix/run it. The PIL renderer (`services/zpl_pil_renderer.py`) is the offline bottom line: covers Code128/GS1-128 via `python-barcode`, DataMatrix via our own ECC200 engine (`services/datamatrix_renderer.py`), text w/ Cyrillic (Arial), `^GB`; it does NOT draw `^GF` raster images, ignores unknown commands, and degrades unsupported barcodes to a labeled placeholder box. `^GF`/`^CI28`/`^A@` are handled by zplr/Labelary, not PIL. New deps: `python-barcode`; `pylibdmtx` is kept only as a test-time decoder reference, not for rendering
- **Own DataMatrix engine:** `services/datamatrix_renderer.py` is a pure-Python ECC200 encoder (ported from ZXing, Apache-2.0). `build_matrix(data, gs_as_fnc1=False)` produces the module matrix; `data_to_image()` renders it. GS (0x1D) is encoded **literally** by default (matches libdmtx bit-for-bit; `gs_as_fnc1=True` would map it to FNC1 232). Always add a `^BX`/`^BXN` field as `^FD{datamatrix}`; the engine is used by the PIL renderer and (via `render_preview_png_local`) by the PDF builder
- **PDF output:** `POST /printing/start` with `printer_id=__pdf__` returns a PDF instead of enqueuing a print job (`services/pdf_renderer.py`, `render_labels_to_pdf()`). It is **fully local** (no Labelary): `render_preview_png_local()` → Pillow PDF, 203 dpi, one page per box, per-box DataMatrix codes. A print-job row is not created for PDF
